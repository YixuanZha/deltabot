...
bool LineFollower::ProcessFrameAndGetInputs(const cv::Mat &frame, std::vector<double> &inputs, double &error_near, double &error_far)
{
    ...
    cv::Rect roi_near_rect(0, frame.rows * 2 / 3, frame.cols, frame.rows / 3); // Near segment at the bottom
    cv::Rect roi_far_rect(0, frame.rows * 1 / 3, frame.cols, frame.rows / 3);  // Far segment in the middle
